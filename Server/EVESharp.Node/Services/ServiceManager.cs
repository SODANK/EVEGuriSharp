    ...
    public PyDataType ServiceCall(string service, string method, ServiceCall call)
    {
        ...
        if (svc is not Service svcInstance)
        {
            System.Console.WriteLine(
                $"[ServiceManager] MissingService: {service}::{method}"
            );
        ...
        System.Console.WriteLine(
            $"[ServiceManager] ServiceCall BEGIN: {service}::{method} " +
            $"sessionRole={call.Session.Role}"
        );
        ...
        switch (svcInstance.AccessLevel)
        {
            case AccessLevel.Location:
                ...
                {
                    System.Console.WriteLine(
                        $"[ServiceManager] UNAUTHORIZED {service}::{method} - " +
                        "missing Session.LOCATION_ID"
                    );
            ...
            case AccessLevel.SolarSystem:
                if (call.Session.ContainsKey(Session.SOLAR_SYSTEM_ID) == false)
                {
                    System.Console.WriteLine(
                        $"[ServiceManager] UNAUTHORIZED {service}::{method} - " +
                        "missing Session.SOLAR_SYSTEM_ID"
                    );
        ...
        try
        {
            PyDataType result = svcInstance.ExecuteCall(method, call);

            System.Console.WriteLine(
                $"[ServiceManager] ServiceCall SUCCESS: {service}::{method} " +
                $"returnedType={result?.GetType().Name ?? "null"}"
            );
        ...
        {
            System.Console.WriteLine(
                $"[ServiceManager] ServiceCall EXCEPTION: {service}::{method} " +
